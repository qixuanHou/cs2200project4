...
static uint64_t find_free_frame(stats_t *stats)
{
	...
	uint64_t f = rlt[0].task_struct->pagetable[rlt[0].vpn].frequency;

	for (uint64_t i = 0; i < (1<<rlt_size); i++) {
		if (f > rlt[i].task_struct->pagetable[rlt[i].vpn].frequency) {
			f = rlt[i].task_struct->pagetable[rlt[i].vpn].frequency;
			victim_pfn = i;
		}
	}
	...
	if (rlt[victim_pfn].task_struct->pagetable[rlt[victim_pfn].vpn].dirty == 1) {
		stats->writes_to_disk = stats->writes_to_disk + 1; 
	}
